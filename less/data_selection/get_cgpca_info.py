...
def get_targeted_gradients(model, target_modules=["v_proj", "o_proj"]):
    ...
    if targeted_grads:
        ...
        # Provide a more detailed error message just in case
...
def format_and_tokenize(prompt_text, response_text, tokenizer, device, max_length=2048):
    """Formats text and masks prompt tokens (-100) for custom anchor processing."""
...
def main():
    parser = argparse.ArgumentParser(description='CG-PCA Extraction and Scoring')
    ...
    # --------------------------------------------------------
    # Phase 1: Contrastive Anchor Profiling
    # --------------------------------------------------------
    ...
    for entry in tqdm(contrastive_data, desc="Computing g_toxic - g_safe"):
        ...
        d_i = g_toxic - g_safe
        difference_grads.append(d_i.numpy())

    print("Computing V_harm via PCA (TruncatedSVD)...")
    D_matrix = np.vstack(difference_grads) 
    svd = TruncatedSVD(n_components=1, random_state=42)
    v_harm_np = svd.fit(D_matrix).components_[0]
    
    # Calibrate Direction
    if np.dot(np.mean(D_matrix, axis=0), v_harm_np) < 0:
        v_harm_np = -v_harm_np

    v_harm = torch.tensor(v_harm_np, dtype=torch.float32, device=device)
    del difference_grads, D_matrix

    # --------------------------------------------------------
    # Phase 2: Candidate Scoring (Cosine Similarity)
    # --------------------------------------------------------
    ...
    for batch in tqdm(train_dataloader, desc="Calculating Cosine Similarity", total=len(train_dataloader)):
        for key in batch: batch[key] = batch[key].to(device)
        
        model.zero_grad()
        model(**batch).loss.backward()
        
        g_j = get_targeted_gradients(model).to(device).to(torch.float32)
        
        # Cosine Similarity
        cos_sim = F.cosine_similarity(g_j.unsqueeze(0), v_harm.unsqueeze(0)).item()
        scores.append(torch.tensor([cos_sim], dtype=torch.float32))
    ...
    print(f"Finished! Cosine Similarity scores saved to {output_file}")
